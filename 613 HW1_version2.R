# exercise 1
# input data
datstu <- read.csv("C:/Users/home/Desktop/class/ECON 613/Xiaomeng_Han_ECON613__HW1/datstu.csv", header=TRUE)
datjss <- read.csv("C:/Users/home/Desktop/class/ECON 613/Xiaomeng_Han_ECON613__HW1/datjss.csv")
datsss <- read.csv("C:/Users/home/Desktop/class/ECON 613/Xiaomeng_Han_ECON613__HW1/datsss.csv")
# calculate the number of students
length(na.omit(datstu$X)) #omit the NAs

# calculate the number of schools
schoolcodenum <- as.vector(as.matrix(datstu[,5:10])) # turn to the vector
...
pgm <- as.vector(as.matrix(datstu[,11:16])) # turn to the vector
...
schoolcode2 <- as.vector(as.matrix(datstu[,5:10]))
pgm2 <- as.vector(as.matrix(datstu[,11:16]))
...
sameschool <- cbind(rep(1:nrow(datstu), each=6), as.vector(as.matrix(datstu[,5:10]))) 
...
sameschool2 <- cbind(rep(1:nrow(datstu), each=6), as.vector(as.matrix(datstu[,5:10])))
...
datstu2 <- datstu[!(is.na(datstu$rankplace)|datstu$rankplace == 99),] 
# take out the data of test score, schoolcode and rankplace
testscoreall <- as.data.frame(datstu2[, c(2, 5:16, 18)])
# get the data of scores of the admitted students
testscore1 <- subset(testscoreall, testscoreall[,14] == 1, select = c(1,2,8))
testscore2 <- subset(testscoreall, testscoreall[,14] == 2, select = c(1,3,9))
testscore3 <- subset(testscoreall, testscoreall[,14] == 3, select = c(1,4,10))
testscore4 <- subset(testscoreall, testscoreall[,14] == 4, select = c(1,5,11))
testscore5 <- subset(testscoreall, testscoreall[,14] == 5, select = c(1,6,12))
testscore6 <- subset(testscoreall, testscoreall[,14] == 6, select = c(1,7,13))
...
minscore <- as.data.frame(aggregate(testscore~schoolcode+program, data = lowscore, FUN = "min"))
meanscore <- as.data.frame(aggregate(testscore~schoolcode+program, data = lowscore, FUN = "mean"))
frequency_1 <- as.data.frame(aggregate(testscore~schoolcode+program, data = lowscore, FUN = "length"))
colnames(minscore)=c("schoolcode","program","lowestscore")
colnames(meanscore)=c("schoolcode","program","averagescore")
colnames(frequency_1)=c("schoolcode","program","size")
...
dataset1 <- merge(dataset1, frequency_1, by.dataset1=c(1,2), by.frenquency_1=c(1,2), all=FALSE,sort=TRUE )
dataset1 <- unique(na.omit(dataset1))
...
tall <- as.data.frame(datstu2[, c(1:2, 5:16, 17:18)])
# get the data of scores of the admitted students
t1 <- subset(tall, tall[,16] == 1, select = c(1,2,3,9,15))
t2 <- subset(tall, tall[,16] == 2, select = c(1,2,4,10,15))
t3 <- subset(tall, tall[,16] == 3, select = c(1,2,5,11,15))
t4 <- subset(tall, tall[,16] == 4, select = c(1,2,6,12,15))
t5 <- subset(tall, tall[,16] == 5, select = c(1,2,7,13,15))
t6 <- subset(tall, tall[,16] == 6, select = c(1,2,8,14,15))
# rename variables and combine the data
...
p_1 <- as.data.frame(datstu2[,c(1,5,11)])
p_1[p_1==""] = NA
p_1 <- na.omit(p_1)                             
p_2 <- as.data.frame(datstu2[,c(1,6,12)])
p_2[p_2==""] = NA
p_2 <- na.omit(p_2)
p_3 <- as.data.frame(datstu2[,c(1,7,13)])
p_3[p_3==""] = NA
p_3 <- na.omit(p_3)
p_4 <- as.data.frame(datstu2[,c(1,8,14)])
p_4[p_4==""] = NA
p_4 <- na.omit(p_4)
p_5 <- as.data.frame(datstu2[,c(1,9,15)])
p_5[p_5==""] = NA
p_5 <- na.omit(p_5)
p_6 <- as.data.frame(datstu2[,c(1,10,16)])
p_6[p_6==""] = NA
p_6 <- na.omit(p_6)
